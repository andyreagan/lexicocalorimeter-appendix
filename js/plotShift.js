...
function plotShift(figure,sortedMag,sortedType,sortedWords,sumTypes,refH,compH) {
    ...
    */
    ...
    canvas.selectAll("p.sumtext")
	.data(["Why ",refH,compH])
	.text(function(d,i) { 
	    if (i==0) {
		// if there are names of the texts, put them here
		if (Math.abs(refH-compH) < 0.01) { return "How the words of reference and "+shiftCompName+" differ";}
		...
		return shiftCompName+" happiness " + (d.toFixed(2));
	    }});
	// .attr("x",width/2+(figwidth-width)/2)
	...
	d3.select(".y.axis.food").call(yAxis);
	d3.selectAll(".tick line").style({'stroke':'black'});
    };
